...
def power_range(wind, height, turb, t_start, t_end):
    ...
    # Filter turbine data where power > 0 and wind speed is between u_in and u_out
...
def calc_aep(filtered, pdf):
    ...
    turb_power_filtered = filtered['Power [kW]'].to_numpy() #filter the power = 0 kW
...
def calc_aep_per_speed(filtered, pdf, coord):
    """
    Calculate Annual Energy Production (AEP) **per wind speed** from turbine and wind distribution data.
    
    Args:
        filtered (pd.DataFrame): Turbine power curve data with columns:
            - 'Wind Speed [m/s]' (wind speeds)
            - 'Power [kW]' (power output at each speed)
        pdf (np.array): Probability density function (PDF) of wind speeds (from Weibull fit).
